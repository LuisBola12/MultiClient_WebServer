...
class Consumer : public virtual Thread {
 ...
 protected:
  ...
  /// True if this consumer owns the queue and it must be deleted in
  /// destructor
  bool ownsQueue;
 ...
 public:
  ...
  explicit Consumer(Queue<DataType>* consumingQueue = nullptr,
    ...
    // Error if asked to create own queue and provided an existing one to
    // use
  ...
  inline Queue<DataType>* getConsumingQueue() { return this->consumingQueue; }
