...
class Socket {
 ...
 public:
    ...
    /// from the socket to the buffer. No data is stored in the internal @a
    /// input buffer. This function may block/ the caller thread if socket is
    /// empty
    ...
    /// Returns true if this socket represents the same connection than the
    /// other
