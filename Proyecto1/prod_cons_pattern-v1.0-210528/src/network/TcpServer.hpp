...
class TcpServer {
 ...
 public:
    ...
    /// Destructor
    ~TcpServer();
    /// Infinetelly listen for client connection requests and accept all of
    /// them. For each accepted connection request, the virtual
    /// onConnectionAccepted() will be called. Inherited classes must override
    /// that method
 ...
 protected:
    ...
    /// Get a socket for the first available address that works for the given
    /// port
