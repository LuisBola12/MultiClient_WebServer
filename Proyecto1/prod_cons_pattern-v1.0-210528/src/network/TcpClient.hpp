...
class TcpClient {
 ...
 public:
    ...
    /// @param server The server's network address, such as a IP (eg: 1.2.3.4)
    /// or a domain name (eg: www.example.net)
    /// @param port The port number where the server is listening for
    /// connections (eg: "8080") or its protocol name (eg: "http")
