...
Socket::Socket() : sharedSocket{new SharedSocket()} {}

void Socket::close() { this->sharedSocket->close(); }
...
ssize_t Socket::readAvailable(char* buffer, size_t capacity) {
    return ::recv(this->sharedSocket->socketFileDescriptor, buffer, capacity,
                  0);
}
...
bool Socket::send() {
    ...
    while (true) {
        const ssize_t result = ::send(this->sharedSocket->socketFileDescriptor,
                                      buffer + sent, length - sent, 0);
...
Socket::operator bool() const {
    return this->sharedSocket->input.good() &&
           this->sharedSocket->output.good();
}
