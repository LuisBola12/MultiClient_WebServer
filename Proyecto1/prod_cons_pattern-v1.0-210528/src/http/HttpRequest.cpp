...
HttpRequest::HttpRequest(const Socket& socket) : HttpMessage(socket) {}

HttpRequest::~HttpRequest() {}

bool HttpRequest::parse() {
    return this->parseRequestLine() && this->parseHeader() && this->parseBody();
}
...
bool HttpRequest::parseBody() {
    ...
    try {
        ...
        if (value.length() > 0) {
            ...
            // TODO(any): stringstream is not suitable to manage large/binary
            // data
